          ...
          if (res.found===1) {
            for (const key in res.res) {
              $("#report_div").append("<table class='table table-responsive' id='"+key+"'> <thead> <th colspan='10'> <center style='background-color:#eee;font-size:20px'>"+res.res[key].branch_name+" </center> </th> </thead> "+
                ...
                "<thead  class='thead-dark'>  <th>#</th>  <th>Date</th>  <th>Product</th>  <th>ClientName</th>  <th>CompanyName</th>  <th>QuantitySold</th> <th>SoldPrice</th>  <th>PaymentMethod</th>  <th>PaymentWay</th>  </thead> </table><tbody> ");
              if (res.res[key].is_found==0) {
                ...
                $("#"+key+"").append("<tr><td>"+ (i+1) +"</td><td>"+res.res[key].data[i].StockOutDate+"</td>  <td>"+res.res[key].data[i].product_name+"</td>  <td>"+res.res[key].data[i].ClientName+"</td>  <td>"+res.res[key].data[i].CompanyName+"</td>  <td>"+Intl.NumberFormat().format(res.res[key].data[i].QuantitySold)+"</td>   <td>"+Intl.NumberFormat().format(res.res[key].data[i].SoldPrice)+"</td>  <td>"+PaymentMethod(res.res[key].data[i].PaymentMethod)+"</td>  <td>"+res.res[key].data[i].PaymentWay+"</td> </tr> ");
                // $("#"+key+"").append("<tr> ");
                ttlQntty+= parseInt(res.res[key].data[i].QuantitySold);
                ttlSoldPrice+= parseInt(res.res[key].data[i].SoldPrice);

                }
                $("#"+key+"").append("<tr><tr><td></td></tr><th colspan='5'>Total</th> <th>"+Intl.NumberFormat().format(ttlQntty)+"</th> <th>"+Intl.NumberFormat().format(ttlSoldPrice)+"</th> <th colspan='2'></th><tr> ");
