      ...
      <div class="modal-dialog" style="min-width: 60%" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" style="font-weight: bold;font-size: 20px;" id="exampleModalLabel">Register Warehouse</h5>
          ...
          <!-- <form action = "employee" method = "POST" name="sentMessage" id="contactForm" novalidate="novalidate"> -->
            ...
            <div class="modal-footer">
                      <div class="form-group">
                        ...
                        <a href="branch"  style="float:right;" class="btn btn-primary">Next</a>
                        <!-- /.input group -->
                      </div>
            </div>
          <!-- </form> -->
          ...
          <div class="box" style="margin-left: 0.5%">
            <div class="box-body">
              <button class="btn btn-success" style="font-weight: bolder;" onclick="return ExportToExcel()">Export Excel</button>
                <button style="float:right;" class="btn btn-primary" id="Next" data-toggle="modal" id="newGoalBtn" data-target="#newRequestModal">New Warehouse</button>
...
//=========================================================== REGISTER WAREHOUSE
  ...
  $("#RegisterWareHouse").click(function(){ 
    var name = $("#name").val();
    if (name=='') {
      ...
      var RegisterWareHouse = true;
      $.ajax({url:"../../../main/action.php",
        type:"GET",data:{RegisterWareHouse:RegisterWareHouse,name:name},cache:false,success:function(res){
          switch(res){
            case 'already':
              $("#respp").addClass("bg-yellow");
              $("#respp").css("display","block");
...
function updateWareHouseModal(name,iid){
  // alert(name);
  document.getElementById("stockName").value=name;
  document.getElementById("stockNameID").value=iid;
  $("#updateStockbtn").click();
}
function deleteWarehouse(iid){
    var deleteWarehouse = true;
    $.ajax({url:"../../../main/action.php",
      type:"GET",data:{deleteWarehouse:deleteWarehouse,iid:iid},cache:false,success:function(res){
        if (res=='used') {
        ...
        type:"POST",data:{available_warehouses:available_warehouses},cache:false,success:function(res){  
          ...
          if (res.found===1) {
            ...
            for (const key in res.res) {
              ...
              var nn = '"'+res.res[key].warehouse_name+'"';
              $("#report_div").append("<tr> <td>"+cnt+".</td> <td>"+res.res[key].warehouse_name+"</td> <td><button class='btn btn-link' onclick='return updateWareHouseModal("+nn+","+res.res[key].warehouse_id+")'> Update </button> <button class='btn btn-danger' onclick='return deleteWarehouse("+res.res[key].warehouse_id+")'>Delete</button></td> </tr>");
              cnt++;
            }
          }else{
...
$("#UpdateBranchBtn").click(function(){
  ...
  if (newBrancName!='') {
    var UpdateWareHouse = true;
    $.ajax({url:"../../../main/action.php",
      type:"GET",data:{UpdateWareHouse:UpdateWareHouse,branchId:branchId,newBrancName:newBrancName},cache:false,success:function(res){
        window.location.reload(true);
      }
    });
  }
